"""Run a local-only cleanroom rerun for weak ProgramBench tasks."""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run a ProgramBench weak-task cleanroom rerun with official eval disabled"
    )
    parser.add_argument("instance_id", help="ProgramBench instance id")
    parser.add_argument(
        "--catalog",
        default=DEFAULT_PROGRAMBENCH_CATALOG,
        help="Path to ProgramBench sample metadata JSON",
    )
    parser.add_argument("--runs", default="runs/weak_task_cleanroom_rerun", help="Local rerun root")
    ...
    parser.add_argument("--min-smoke-contract-axes", type=_non_negative_int, default=0)
    parser.add_argument(
